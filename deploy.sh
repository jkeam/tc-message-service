...
make_task_def(){
   ...
   "containerDefinitions": [
    {
      ...
      "environment": [
        ...
        {
          "name": "DDB_DISCOURSE_WEBHOOK_LOGS_TABLE",
          "value": "%s"
        },
        {
  ...
  DDB_DISCOURSE_WEBHOOK_LOGS_TABLE=$(eval "echo \$${ENV}_DDB_DISCOURSE_WEBHOOK_LOGS_TABLE")
  DISCOURSE_WEBHOOK_SECRET=$(eval "echo \$${ENV}_DISCOURSE_WEBHOOK_SECRET")
  if [ "$ENV" = "PROD" ]; then
    NODE_ENV=production
  elif [ "$ENV" = "DEV" ]; then
  ...
  task_def=$(printf "$task_template" $family $ACCOUNT_ID $AWS_ECS_CONTAINER_NAME $ACCOUNT_ID $AWS_REGION $AWS_REPOSITORY $CIRCLE_SHA1 $NODE_ENV $LOG_LEVEL $CAPTURE_LOGS $LOGENTRIES_TOKEN $API_VERSION $AWS_REGION $AWS_ACCESS_KEY_ID $AWS_SECRET_ACCESS_KEY $AUTH_DOMAIN $AUTH_SECRET $DB_MASTER_URL $RABBITMQ_URL $MEMBER_SERVICE_URL $IDENTITY_SERVICE_ENDPOINT $SYSTEM_USER_CLIENT_ID $SYSTEM_USER_CLIENT_SECRET $DDB_DISCOURSE_WEBHOOK_LOGS_TABLE $DISCOURSE_WEBHOOK_SECRET $PORT $PORT $AWS_ECS_CLUSTER $AWS_REGION $NODE_ENV)
}
